...
def userbookinglist():
    ...
    print(rows)
    msg = "LIST OF ALL BUSES"
    return render_template("busBookList.html", rows=rows, msg=msg, user=username, fl=flag)
    ...
    if request.method == 'POST':
        try:
            ...
            with sql.connect("database.db") as con:
                ...
                cur.execute("SELECT * FROM user WHERE name=(?) AND pass=(?) ", (username, pw))
                rows = cur.fetchone();
                print("rows in booking: ", rows)
                user = rows[0]
                un = rows[1]
                print("nop:", nop)
                print("bus price: ", bus[5])
                price = nop * bus[5]
                print("price: ", price)
                # cur.execute("INSERT INTO book (busid,busname,userid,username,nopass) VALUES (?,?,?,?,?)",
                #             (idbus, busn, user, un, nop))
                cur.execute("INSERT INTO book (userid, username, busid, busname, noSeats, price) VALUES (?,?,?,?,?,?)", (user, un, idbus, busn, nop, price))
                if nop<=num_seats:
